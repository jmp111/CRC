...
load('CRC_microbiome_data_proc.mat', 'C', 'M_data', 'M_labs', 'i_Tumour')
...
level_taxonomy_xaxis=5; % order
level_taxonomy_yaxis=8; % species
...
ylim([0.5 nr+0.5])
hold off
set(gca,'XTick',1:numel(op),'XTickLabel',M_labs(op,level_taxonomy_xaxis),'YTick',1:numel(op),'YTickLabel',M_labs(op,level_taxonomy_yaxis),'FontSize',7)
XTickRotateJMP(M_labs(op,level_taxonomy_xaxis),67.5)
...
Mc_data=M_data./repmat(sum(M_data,2),1,numel(ci)); % compositional data
...
load('KRCA_microbiome_data_proc.mat', 'C', 'M_data', 'M_labs', 'i_Tumour')
...
load('KRCA_microbiome_data_proc_paired.mat', 'K_pid', 'M_data', 'M_labs', 'M_pid', 'i_Tumour')
